package com.developer.kimy.data.source.local;
...
public class MedicineDBHelper extends SQLiteOpenHelper {
    ...
    /**
     * Pill table columns, used by PillHistory Table
     */
    private static final String KEY_PILLNAME = "pillName";

    /**
     * Alarm table columns, Hour & Minute used by PillHistory Table
     */
    ...
    /**
     * PillHistory Table columns, some used above
     */
...
// ############################## create methods ###################################### //
    ...
    /**
     * uses a pillHistory model object to store histories in the DB
     *
     * @param pillHistory a pillHistory model object
     */
    public void createHistory(PillHistory pillHistory) {
        SQLiteDatabase db = getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_PILLNAME, pillHistory.getPillName());
        values.put(KEY_DATE_STRING, pillHistory.getDateString());
        values.put(KEY_HOUR, pillHistory.getHourTaken());
        values.put(KEY_MINUTE, pillHistory.getMinuteTaken());
        values.put(KEY_DOSE_QUANTITY, pillHistory.getDoseQuantity());
        values.put(KEY_DOSE_UNITS, pillHistory.getDoseUnit());
        values.put(KEY_ACTION, pillHistory.getAction());
        values.put(KEY_ALARM_ID, pillHistory.getAlarmId());
...
// ############################# get methods ####################################### //
    ...
    /**
     * allows pillBox to retrieve from PillHistory table
     *
     * @return a list of all history objects
     */
    public List<PillHistory> getHistory() {
        List<PillHistory> allPillHistory = new ArrayList<>();
        ...
        if (c.moveToFirst()) {
            do {
                ...
                allPillHistory.add(h);
            } while (c.moveToNext());
        }
        c.close();
        return allPillHistory;
    }
